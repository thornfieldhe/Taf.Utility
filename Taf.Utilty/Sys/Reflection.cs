...
namespace Taf.Utility{
    ...
    #region 获取实例信息
        ...
        public static IList<Tuple<string, string>> GetMembers<T>(){
            ...
            members.IfNotNull(
                r => {
                    r.ForEach(
                        i => { result.Add(new Tuple<string, string>(i.Name, i.PropertyType.Name)); });
                });
            return result;
        }
    ...
    #region CreateInstance(动态创建实例)
        ...
        public static T CreateInstance<T>(Type type, params object[] parameters) =>
            Activator.CreateInstance(type, parameters).To<T>();
    ...
    #region GetMemberDescription(获取描述)
        ...
        public static string GetMemberDescription<T>(string memberName) =>
            GetMemberDescription(GetType<T>(), memberName);
        ...
        public static string GetMemberDescription(Type type, PropertyInfo property){
            ...
            var attribute =
                property.GetCustomAttributes(typeof(DescriptionAttribute), true).FirstOrDefault() as

    ...
    public partial class Extensions{
        ...
        public static string ToScienceString(this double v, int sd){
            ...
            if(result.Contains("E")){
                ...
                && v < 1){
                    //是小于1的数字
                    var noExistStr = result.Substring(index + 1).TrimStart('0');
                    var isE        = result.Substring(index + 1).Length - noExistStr.Length + 1 > sd;
                    if(isE){
                        result = v.ToString("E" + sd);
                    } else if(result.Substring(index + 1).Length > sd){
                        result = Math.Round(v, sd).ToString();
                    }
                } else if(result.Substring(0, index).Length > sd){
                    result = v.ToString("E" + sd);
                } else if(result.Substring(index + 1).Length > sd){
                    result = Math.Round(v, sd).ToString();
                }
            } else if(result.Length > sd){
                result = v.ToString("E" + sd);
            }
        ...
        public static string ToPercentString(this double v, int sd = 2){
            if(v == 0)
                return "0%";
            var result = v.ToString("P" + sd);
            return Regex.IsMatch(result, @"^0\.00+") ? "0%" : result;
        }

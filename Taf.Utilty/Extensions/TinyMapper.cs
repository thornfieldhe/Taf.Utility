// --------------------------------------------------------------------------------------------------------------------
// <copyright file="TinyMapper.cs" company="" author="何翔华">
//   
// </copyright>
// <summary>
//   通过Emit 实现对象映射
...
namespace Taf.Utility{
    ...
    ///     通过Emit 实现对象映射
    /// </summary>
    public class TinyMapper<TSource, TTarget> where TSource : class where TTarget : class, new(){
        public static readonly Func<TSource, TTarget> MapFunc = GetMapFunc();

        public static readonly Action<TSource, TTarget> MapAction = GetMapAction();
        ...
        private static Action<TSource, TTarget> GetMapAction(){
            ...
            //过滤忽略项
            if(_config.IgnoreColoums       != null
            && _config.IgnoreColoums.Count > 0){
                targetTypes = targetTypes.Where(x => !_config.IgnoreColoums.Contains(x.Name));
            }

            var sourceTypes = sourceType.GetProperties()
                                        .Where(x => x.GetIndexParameters().Length == 0
                                                 && (x.PropertyType.IsPublic || x.PropertyType.IsNestedPublic)
                                                 && x.CanRead);
            foreach(var targetItem in targetTypes){
                var sourceItem =
                    sourceTypes.FirstOrDefault(x => string.Compare(x.Name, targetItem.Name, _config.IgnoreCase) == 0);

                //判断实体的读写权限
                if(sourceItem == null
                || !sourceItem.CanRead
                || sourceItem.PropertyType.IsNotPublic){
                    continue;
                }
                ...
                //当非值类型且类型不相同时
                if(!sourceItem.PropertyType.IsValueType
                && sourceItem.PropertyType != targetItem.PropertyType
                && sourceItem.PropertyType != typeof(string)
                && targetItem.PropertyType != typeof(string)
                ){
                    //判断都是(非泛型、非数组)class
                    if(sourceItem.PropertyType.IsClass
                    && targetItem.PropertyType.IsClass
                    && !sourceItem.PropertyType.IsArray
                    && !targetItem.PropertyType.IsArray
                    && !sourceItem.PropertyType.IsGenericType
                    && !targetItem.PropertyType.IsGenericType){
                        var expression =
                            GetClassExpression(sourceProperty, sourceItem.PropertyType, targetItem.PropertyType);
                    ...
                    && typeof(IEnumerable).IsAssignableFrom(targetItem.PropertyType)){
                        var expression =
                            GetListExpression(sourceProperty, sourceItem.PropertyType, targetItem.PropertyType);
                        expressions.Add(Assign(targetProperty, expression));
                    }
                ...
                if(IsNullableType(sourceItem.PropertyType)
                && !IsNullableType(targetItem.PropertyType)){
                    var hasValueExpression = Equal(Property(sourceProperty, "HasValue"), Constant(true));
                    var conditionItem = Condition(hasValueExpression, Convert(sourceProperty, targetItem.PropertyType)
                                                , Default(targetItem.PropertyType));
        ...
        private static Expression GetClassExpression(Expression sourceProperty, Type sourceType, Type targetType){
            ...
            var mapperType = typeof(TinyMapper<,>).MakeGenericType(sourceType, targetType);
            var actionType = typeof(Action<,>).MakeGenericType(sourceType, targetType);

            var iftrue = Call(mapperType.GetMethod(nameof(Map), new[]{ sourceType, actionType }), sourceProperty
                            , Constant(null, actionType));
        ...
        private static Expression GetListExpression(Expression sourceProperty, Type sourceType, Type targetType){
            ...
            var mapperType = typeof(TinyMapper<,>).MakeGenericType(sourceArg, targetArg);
            var actionType = typeof(Action<,>).MakeGenericType(sourceArg, targetArg);

            var mapperExecMap = Call(mapperType.GetMethod(nameof(MapList), new[]{ sourceType, actionType })
                                   , sourceProperty, Constant(null, actionType));
            ...
            {
                iftrue = Call(typeof(Enumerable), nameof(Enumerable.ToArray)
                            , new[]{ mapperExecMap.Type.GenericTypeArguments[0] }, mapperExecMap);
